...
This is a STARTER app for quick standalone viewing/tuning. It IS also registered
as a selectable detector in the merged scanner (see server/detectors.py), so it
does not need to be running for scans on port 5050 to use the HR4000. Run it on
its own for a dedicated live-spectrum viewer:
...
def api_export():
    ...
    return plots.zip_response(files, "hr4000_export")
